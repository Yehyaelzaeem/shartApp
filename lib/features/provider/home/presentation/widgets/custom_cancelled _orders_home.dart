...
class CustomCancelledOrderHomeWidget extends StatelessWidget {
  const CustomCancelledOrderHomeWidget({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ProviderOrdersCubit, ProviderOrdersState>(
      ...
      builder: (BuildContext context,ProviderOrdersState state) {
         if(cubit.myOrdersCancelled!=null){
           if(cubit.myOrdersCancelled!.data!.length==0){
             return Padding(
               ...
               itemBuilder: (BuildContext context, int index) {
                 return InkWell(
                   ...
                   child: CustomProductMyOrderWidget(
                     isHome: true,
                     providerOrderModelData: cubit.myOrdersCancelled!.data![index],
                   ),
                 );
               },
               shrinkWrap: true,
               physics: NeverScrollableScrollPhysics(),
               itemCount: cubit.myOrdersCancelled!.data!.length,
             );
           }
         }else{
           return Padding(
